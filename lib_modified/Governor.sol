    ...
    Groth16Verifier
{
    ...
    constructor(
        ...
        MerkleTreeWithHistory(levels) // 20 levels for the Merkle tree
    {
        _name = name_;
        _verifier = Groth16Verifier(verifierAddress);
    }
    ...
    function queue(
        ...
        delete _proposalTrees[proposalId];
    ...
    function castVote(
        ...
        return _castVote(proposalId, _proof, nullfier, support, "");
    }
    ...
    function castVoteWithReason(
        ...
        return _castVote(proposalId, _proof, nullfier, support, reason);
    }
    ...
    function castVoteWithReasonAndParams(
        ...
        return _castVote(proposalId, _proof, nullfier, support, reason, params);
    }
    ...
    function summitVote(uint256 proposalId, uint256 commitment) public returns (uint256) {
        ...
        if (_hasSummited[proposalId][msg.sender]) {
            revert("Governor: already summited");
        }
        if (weight == 0) {
            revert("Governor: no votes available");
        }
        _hasSummited[proposalId][msg.sender] = true;
        _insert(proposalId, commitment);
    }
